...
const char * const BITCOIN_CONF_FILENAME = "kittycash.conf";
...
#else
    const char* pszModule = "kittycash";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KittyCash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/KittyCash";
#else
    // Unix-like
    return pathRet / ".kittycash";
#endif
#endif
}
