...
- [Development guidelines](#development-guidelines)
    - [General KittyCash Core](#general-kittycash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
KittyCash Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play kittycashs" on the test network, if you
...
KittyCash Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_kittycash
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_kittycash --log_level=test_suite
$ valgrind -v --leak-check=full src/kittycashd -printtoconsole
...
# A coverage report will now be accessible at `./test_kittycash.coverage/index.html`.
```
...
To profile a running kittycashd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep kittycashd` -- sleep 60
```
...
KittyCash Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as KittyCash Core, where everyone uses
...
pay attention to for reviewers of KittyCash Core code.

General KittyCash Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: KittyCash Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of KittyCash Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against KittyCash Core so that they can be integrated
...
file descriptors, it will cause problems with KittyCash's `select()` loop, because
...
compatibility between nodes. This happened in KittyCash 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to KittyCash's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `kittycash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `kittycash-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of KittyCash Core, the wallet was always
