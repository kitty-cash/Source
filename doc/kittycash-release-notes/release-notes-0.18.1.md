KittyCash Core version 0.18.1 is now available from:

  <https://download.kittycash.org/kittycash-0.18.1/>
...
`/Applications/KittyCash-Qt` (on Mac) or `kittycashd`/`kittycash-qt` (on
Linux).
...
KittyCash Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use KittyCash Core on unsupported systems.
...
10.10. Additionally, KittyCash Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:24507:24507` (this is an extra `:24507` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@kittycash.org) the KittyCash Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 PURR),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in PURR per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/kittycash-conf.md)
  about the `kittycash.conf` file describes how to use it to configure
  KittyCash Core.

- A new document introduces KittyCash Core's BIP174 [Partially-Signed
  KittyCash Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent KittyCash-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  KittyCash Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when KittyCash Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of KittyCash Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart kittycashd with the `-deprecatedrpc=generate`
...
- A new `kittycash-wallet` tool is now distributed alongside KittyCash
...
This section describes planned changes to KittyCash Core that may affect
other KittyCash software and services.

- Since version 0.16.0, KittyCash Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with KittyCash Core 0.20 (expected about
  a year after 0.18), KittyCash Core will default to native segwit
  ...
  to bech32 addresses, and if the KittyCash Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in KittyCash Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any KittyCash Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, KittyCash Core
...
- A sub-project of KittyCash Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with KittyCash Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to KittyCash Core's own implementation, although entropy
  gathered by KittyCash Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves KittyCash Core a
...
- On macOS, KittyCash Core now opts out of application CPU throttling
...
- #24507 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
