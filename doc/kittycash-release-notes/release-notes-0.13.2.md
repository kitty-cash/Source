KittyCash Core version 0.13.2 is now available from:

  <https://download.kittycash.org/kittycash-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a kittycash
...
ECDSA signatures inside KittyCash transactions now use validation using
...
Previous versions of KittyCash Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of KittyCash Core was to raise the default minimum
relay fee.

KittyCash Core 0.13.2 will have a strict maximum size on the mempool. The
...
KittyCash Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running kittycashd without having to do any manual configuration.
...
KittyCash Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. KittyCash Core will relay transactions with
...
entirely in KittyCash Core 0.13, and it is at this time undecided whether the
...
KittyCash Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
KittyCash Core automatically creates a hidden service to listen on, without
manual configuration. KittyCash Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if KittyCash Core is listening, and
...
KittyCashd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals KittyCash
Core to use floating fees. By default, KittyCash Core will use floating
fees.
...
(default: `0.0002` PURR/kB).

At all times, KittyCash Core will cap fees at `-maxtxfee=<x>` (default:
0.10) PURR.
Furthermore, KittyCash Core will never create transactions paying less than
...
`kittycash.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 224507 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 224507 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /kittycashrpc>
        ProxyPass http://127.0.0.1:24507/
        ProxyPassReverse http://127.0.0.1:24507/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass kittycashd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< kittycashrpc:<password>
    </Location>
...
Restarting kittycashd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in kittycash.conf
...
kittycash-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/kittycash-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The KittyCash Core code base has
...
- `kittycash-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `kittycash-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the KittyCash scripting system that in combination with
...
KittyCash transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of KittyCash Core.
...
The two phases are distinct in the KittyCash-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming KittyCash wallet
  software more difficult and which seriously complicates the design of smart
  contracts for KittyCash.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each KittyCash block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of kittycashs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of kittycashs being spent is exactly
...
- **Increased security for multisig:** KittyCash addresses (both P2PKH addresses
  ...
  believed to be provided by KittyCash's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of KittyCash's current ledger (called the UTXO set),
  ...
  note that KittyCash Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  KittyCash users to individually opt-in to almost any change in the KittyCash
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and KittyCash Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the KittyCash
client to KittyCash Core 0.13.2 to signal for segwit support. When a
...
Since KittyCash Core 0.10.0, nodes have defaulted to only relaying and
...
- `kittycash-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `kittycash-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
