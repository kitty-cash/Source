# Fuzzing KittyCash Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing KittyCash Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
Every single pull request submitted against the KittyCash Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make KittyCash Core more robust.
...
with certain assembly code from KittyCash Core's code. See [developer notes on sanitizers](/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing KittyCash Core using american fuzzy lop (`afl-fuzz`)

## Quickstart guide

To quickly get started fuzzing KittyCash Core using [`afl-fuzz`](https://github.com/google/afl):
