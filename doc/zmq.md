...
The KittyCash Core daemon can be configured to act as a trusted "border
router", implementing the kittycash wire protocol and relay, making
...
The ZeroMQ feature in KittyCash Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building kittycashd:
...
For instance:
    ...
    $ kittycashd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/kittycashd.tx.raw \
...
These options can also be provided in kittycash.conf.
...
From the perspective of kittycashd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into kittycashd directly. Furthermore, no information is
...
using. KittyCashd appends an up-counting sequence number to each
